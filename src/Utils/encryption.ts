 ...
 // تحويل النص إلى مشفر
  encryption = (token :string , key:string) => {
    ...
    for (let i = 0; i < token.length; i++) {
        ...
        const startIndex = token.length % key.length;
        const keyCharCode = key.charCodeAt((i + startIndex) % key.length);
...
// فك التشفير
 decryption = (encryptedToken:string, key:string) => {
    // فك خلط الحروف
    let reversedToken = encryptedToken.split("").reverse().join("");
    
    let decrypted = "";
    for (let i = 0; i < reversedToken.length; i++) {
        const offsetCharCode = reversedToken.charCodeAt(i);
        const startIndex = encryptedToken.length % key.length;
        const keyCharCode = key.charCodeAt((i + startIndex) % key.length);
